...
correlations <- data.frame(read.table(paste0(output_dir, "/results/sponging/filtered_circRNA_miRNA_correlation.tsv"), sep = "\t", stringsAsFactors = F, header = T))
...
plotCorrelationDistribution <- function(correlations_df, filter_criteria_string, plot_folder, plot_name){
  ...
  p + geom_vline(xintercept = mean(correlations_df$pearson_R), linetype="dashed", 
                 color = "black", size=0.7) + geom_text(aes(x=mean(correlations_df$pearson_R), label=paste(round(mean(correlations_df$pearson_R), digits = 3), "\n"), y = y_coord), vjust = 1.25, angle=90)
...
plotCorrelationDistribution(correlations_processed, paste0("Filter: binding sites > ", bind_sites_filter), plot_folder, paste0("correlation_distribution_minBindSites_", bind_sites_filter, "_AND"))
...
plotCorrelationDistribution(correlations_processed, paste("Filter: pval <", pval_filter, ", normRSS >", maxRSS, ", bind_sites >", bind_sites_filter), plot_folder, paste0("correlation_distribution_minBindSites_", bind_sites_filter, "_AND_pval_", pval_filter,"AND_RSS_", maxRSS))
...
plotCorrelationForPair <- function(circRNA, miRNA, circRNA_expression_df, miRNA_expression_df, plot_folder, plot_name){
  ...
  ggscatter(joined_counts, y = "miRNA_counts", x = "circRNA_counts",
            ...
            conf.int = TRUE) + stat_cor(method = "pearson") + ggtitle(paste(chr, ":", start,"-", end ," VS. ", miRNA, sep="")) +
...
for (i in 1:10){
  ...
  plotCorrelationForPair(circRNA_min, miRNA_min, circRNA_expression, miRNA_expression, plot_folder, paste0("correlation_pair_", circRNA_min, "_", miRNA_min))
